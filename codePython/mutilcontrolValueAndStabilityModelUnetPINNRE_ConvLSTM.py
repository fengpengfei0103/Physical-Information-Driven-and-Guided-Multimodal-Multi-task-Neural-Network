...
#             nn.Conv2d(n_feats // 2, n_feats // 2, 3, 1, 1, groups=n_feats // 2),
#             nn.Conv2d(n_feats // 2, n_feats // 2, 5, stride=1, padding=(5 // 2) * 2, groups=n_feats // 2, dilation=2),
#             nn.Conv2d(n_feats // 2, n_feats // 2, 1, 1, 0))
#
#         self.X3 = nn.Conv2d(n_feats // 2, n_feats // 2, 3, 1, 1, groups=n_feats // 2)
...
class MLKA(nn.Module):
    ...
    def forward(self, x):
        ...
        a = torch.cat([self.LKA1(a_1) * self.X1(a_1), self.LKA3(a_2) * self.X3(a_2),
                       self.LKA5(a_3) * self.X5(a_3),
...
class ConvLSTM(nn.Module):
    def __init__(self, input_channels, hidden_channels, kernel_size=3):
        ...
        # 门控机制的各部分卷积
        self.conv_xi = nn.Conv2d(input_channels, hidden_channels, kernel_size, padding=self.padding)
        self.conv_hi = nn.Conv2d(hidden_channels, hidden_channels, kernel_size, padding=self.padding, bias=False)
        self.conv_xf = nn.Conv2d(input_channels, hidden_channels, kernel_size, padding=self.padding)
        self.conv_hf = nn.Conv2d(hidden_channels, hidden_channels, kernel_size, padding=self.padding, bias=False)
        self.conv_xo = nn.Conv2d(input_channels, hidden_channels, kernel_size, padding=self.padding)
        self.conv_ho = nn.Conv2d(hidden_channels, hidden_channels, kernel_size, padding=self.padding, bias=False)
        self.conv_xg = nn.Conv2d(input_channels, hidden_channels, kernel_size, padding=self.padding)
        self.conv_hg = nn.Conv2d(hidden_channels, hidden_channels, kernel_size, padding=self.padding, bias=False)
    ...
    def forward(self, x, hidden_state=None):
        ...
        g = torch.tanh(self.conv_xg(x) + self.conv_hg(h))
...
class FeatureModule(nn.Module):
    def __init__(self):
        ...
        self.ConvNorm1 = ConvNorm(in_channels=8, out_channels=8, kernel_size=3, stride=1,
...
class SlopeNet(nn.Module):
    def __init__(self):
        ...
        # 原编码器部分保持不变
        self.EncoderConvNorm1 = ConvNorm(in_channels=3, out_channels=16, kernel_size=3, stride=1,
                                         leaky=False, norm='INSTANCE', activation=True)
        self.EncoderMaxPool = nn.MaxPool2d(2, stride=2)
        self.EncoderConvNorm2 = ResBlock(in_channels=16, out_channels=16, stride=1,
                                         leaky=False, norm='INSTANCE')
        self.MAB = MAB(16)
        ...
        self.decodeConvNorm1 = ConvNorm(in_channels=16, out_channels=16, kernel_size=3,
                                        stride=1, leaky=False, norm='INSTANCE', activation=True)
        ...
        self.stabilyBasicModule = nn.Sequential(
            ConvNorm(in_channels=16, out_channels=32, kernel_size=3, stride=1,
                     leaky=False, norm='INSTANCE', activation=True),
            nn.MaxPool2d(2, stride=2),
            CBAM(32),
            ConvNorm(in_channels=32, out_channels=16, kernel_size=3, stride=1,
                     leaky=False, norm='INSTANCE', activation=True),
            nn.MaxPool2d(2, stride=2),
            CBAM(16),
        )
        ...
        # 替换全连接层为ConvLSTM序列
        self.conv_lstm_stack = nn.Sequential(
            ConvLSTM(input_channels=16, hidden_channels=128, kernel_size=3),
            nn.MaxPool2d(2),  # 下采样
            ConvLSTM(input_channels=128, hidden_channels=2, kernel_size=3),
            nn.MaxPool2d(2),  # 下采样
            ConvLSTM(input_channels=2, hidden_channels=1, kernel_size=3)
        )
        ...
        self.log_var3 = nn.Parameter(torch.zeros(1))
    ...
    def compute_Saturability_PorePressure(self, Sr):
        ...
        term = torch.pow(S_clamp, -1 / m) - 1
        term = torch.pow(term, 1 / n)
        psi_prime = (-1 / alpha) * term
    ...
    def forward(self, x, featureinit, PorePressure, Stress):
        ...
        temp = x * feature
        x = x + temp
        PorePressureControl = (PorePressureControl + PorePressureControl * feature +
                               self.compute_Saturability_PorePressure(x)) / 2
        ...
        # 通过ConvLSTM序列处理
        h, _ = self.conv_lstm_stack[0](Stabilization)
        h = self.conv_lstm_stack[1](h)
        h, _ = self.conv_lstm_stack[2](h)
        h = self.conv_lstm_stack[3](h)
        h, _ = self.conv_lstm_stack[4](h)

        # 全局平均池化得到标量输出
        Stabilization = self.adaptive_pool(h).squeeze(-1).squeeze(-1)

        # 添加fv的影响（调整fv形状以匹配）
        fv = fv.view(fv.size(0), -1)
        if fv.size(1) > 1:  # 如果fv维度不匹配，取平均值
            fv = fv.mean(dim=1, keepdim=True)
        Stabilization = Stabilization + fv

        return x, PorePressureControl, StressControl, Stabilization

    def get_parameter_groups(self):
        # 共享参数（影响 x, PorePressureControl, StressControl）
        shared_params = []
        # 编码器部分
        shared_params += list(self.EncoderConvNorm1.parameters())
        shared_params += list(self.EncoderConvNorm2.parameters())
        shared_params += list(self.MAB.parameters())
        shared_params += list(self.bridge.parameters())
        # 解码器部分
        shared_params += list(self.decoderConvTrans.parameters())
        shared_params += list(self.decodeConvNorm1.parameters())
        shared_params += list(self.final_conv.parameters())
        # 任务特定模块
        shared_params += list(self.FeatureModule.parameters())
        shared_params += list(self.PorePressureControlModule.parameters())
        shared_params += list(self.StressControlModule.parameters())

        # Stabilization 专用参数
        stabilization_params = []
        stabilization_params += list(self.stabilyBasicModule.parameters())
        stabilization_params += list(self.conv_lstm_stack.parameters())
        stabilization_params += list(self.adaptive_pool.parameters())
        stabilization_params += list(self.FeatureModuleStab.parameters())

        return shared_params, stabilization_params

# 定义设备
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# 损失函数
criterion_image = nn.MSELoss()
criterion_numeric = nn.SmoothL1Loss()   # 数值任务损失

def color_to_value(img, vmin, vmax):
    """将彩虹色图像转换为物理值"""
    # 假设rainbow颜色条从蓝(0)到红(1)
    img = img.detach().cpu().numpy().transpose(0, 2, 3, 1)
    # img = img.cpu().numpy().transpose(1, 2, 0)
    gray = 0.299 * img[...,0] + 0.587 * img[...,1] + 0.114 * img[...,2]  # RGB转灰度
    normalized = (gray - gray.min()) / (gray.max() - gray.min())  # 归一化到[0,1]
    return vmin + normalized * (vmax - vmin)  # 映射到实际范围
# 自定义损失函数
def modelLoss(net, XTrain1, XTrain2, XTrain3, XTrain4, Y1, Y2, Y3, Y5, epoch, i):
    ...
    if epoch is not None and epoch % 10 == 0 and i is not None:
        ...
        save_dir = "MutilControl训练过程图像/PINN饱和度StabilityUncertaintyWeighting"
        ...
        output_img = (output_img * 255).astype(np.uint8)
        save_dir = "MutilControl训练过程图像/PINN孔隙压力StabilityUncertaintyWeighting"
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, f"Train_epoch_{epoch + 1}_{i + 1}.png")
        plt.imsave(save_path, output_img)
        ...
        output_img = (output_img * 255).astype(np.uint8)
        save_dir = "MutilControl训练过程图像/PINN应力StabilityUncertaintyWeighting"
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, f"Train_epoch_{epoch + 1}_{i + 1}.png")
        plt.imsave(save_path, output_img)

        with open('MutilControl训练过程图像/PINNUncertaintyWeightingtrain_results.txt','a') as file:
            file.write(f'Train_epoch_{epoch + 1}_{i + 1}_{outputs5}\n')

    P_Y1 = color_to_value(Y1,0,1)
    P_outputs1 = color_to_value(outputs1, 0, 1)

    P_Y2 = color_to_value(Y2, -400000, 200000)
    P_outputs2 = color_to_value(outputs2, -400000, 200000)

    P_Y3 = color_to_value(Y3, 0, 300000)
    P_outputs3 = color_to_value(outputs3, 0, 300000)

    # 数据驱动数值计算损失
    loss1 = criterion_image(outputs1, Y1)
    loss2 = criterion_image(outputs2, Y2)
    loss3 = criterion_image(outputs3, Y3)
    loss5 = criterion_image(outputs5.view(-1).float(), Y5.float())

    # 1. 有效应力原理约束
    stress_loss = criterion_image(torch.from_numpy(P_outputs3[...,:200,:200] +
                                  P_outputs1[...,:200,:200] * P_outputs2[...,:200,:200]).float().to(device),
                                  torch.from_numpy(P_Y3[...,:200,:200] +
                                  P_Y1[...,:200,:200] * P_Y2[...,:200,:200]).float().to(device)) / (400000 * 20000)

    # 2. 达西定律约束
    pore_pressure = outputs2[...,:200,:200]
    grad_px, grad_py = compute_gradient(pore_pressure, dx = 1.0, dy = 1.0)
    darcy_loss = torch.mean(grad_px ** 2 + grad_py ** 2)

    # 3. 质量守恒约束
    saturation = outputs1[...,:200,:200]
    grad_sx, grad_sy = compute_gradient(saturation, dx = 1.0, dy = 1.0)
    continuity_loss = torch.mean((grad_sx + grad_sy) ** 2)

    # 不确定性系数调整损失权重
    # weighted_loss1 = torch.exp(-net.log_var1) * (loss1 + loss2 + loss3) + net.log_var1
    # # weighted_loss2 = torch.exp(-net.log_var2) * loss5 + net.log_var2
    # weighted_loss3 = torch.exp(-net.log_var3) * (stress_loss + darcy_loss + continuity_loss) + net.log_var3
    weighted_loss1 = torch.exp(-net.log_var1) * (loss1 + loss2 + loss3)
    # weighted_loss2 = torch.exp(-net.log_var2) * loss5 + net.log_var2
    weighted_loss3 = torch.exp(-net.log_var3) * (stress_loss + darcy_loss + continuity_loss)
    print("log_var1,2:",net.log_var1.item(),net.log_var3.item())

    loss = weighted_loss1 + weighted_loss3
    ...
    return loss, loss5, stress_loss, darcy_loss, continuity_loss, gradients, {}
...
def train_model(datasetTrain, model_save_dir='models', batch_size=4, epochs=500, lr=0.01):
    ...
    # 获取参数组
    shared_params, stabilization_params = model.get_parameter_groups()
    # 优化器
    optimizer = torch.optim.Adam(list(model.parameters()), lr=lr)
    optimizer_stab = torch.optim.Adam(stabilization_params, lr=lr)  # Stabilization 优化器
    # 训练
    num_epochs = epochs
    model.train()
    for epoch in range(num_epochs):
        ...
        for i, (x1, x2, x3, x4, y1, y2, y3, y5) in enumerate(dataloader):
            ...
            # 计算损失
            loss, loss5, stress_loss, darcy_loss, continuity_loss, gradients, _ = modelLoss(model, x1, x2, x3, x4, y1, y2, y3, y5, epoch, i)
            ...
            # 更新 Stabilization 专用参数
            optimizer_stab.zero_grad()
            loss5.backward()  # 不保留计算图
            optimizer_stab.step()

            running_loss += loss.item()

            elapsed_time = timedelta(seconds=int(time.time() - start_time))
            print(f"Epoch [{epoch + 1}/{num_epochs}] Batch [{i + 1}] Time [{elapsed_time}],"
                  f" Loss: {loss.item():.4f}, Loss5: {loss5.item():.4f}, stress_loss: {stress_loss.item():.4f},"
                  f" darcy_loss: {darcy_loss.item():.4f}, continuity_loss: {continuity_loss.item():.4f}")
        ...
        if (epoch + 1) % 100 == 0:
            ...
            save_mode_path = os.path.join(snapshot_path,
                                          'ConvLSTMPINNMutilModelUnetStability_epoch_' +
                                          str(epoch + 1) + '.pth')
...
def rtest_model(datasetTest, InfiltrationStrength, model_path, output_dir='output', batch_size=1):
    ...
    with ((torch.no_grad())):
        for i, (x1, x2, x3, x4, y1, y2, y3, y5 ) in enumerate(dataloaderTest):
            ...
            loss = criterion_image(outputs1, y1) + criterion_image(outputs2, y2) + criterion_image(outputs3, y3) + criterion_image(outputs5.squeeze(0), y5)
            test_loss += loss.item()
            ...
            output_img = outputs1[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/ConvLSTMPINN饱和度StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            ...
            output_img = outputs2[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/ConvLSTMPINN孔隙压力StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"prediction_{InfiltrationStrength[i]}_{(Time[i] + 1)}.png")
            plt.imsave(save_path, output_img)
            ...
            output_img = outputs3[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/ConvLSTMPINN应力StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"prediction_{InfiltrationStrength[i]}_{(Time[i] + 1)}.png")
            plt.imsave(save_path, output_img)
            ...
            # with open(f'{output_dir}/PINNUncertaintyWeightingprediction_results.txt','a') as file:
            #     file.write(f'Prediction_{InfiltrationStrength[i]}_{(Time[i] + 1)}_{outputs5.item()}_{y5.item()}\n')
            # 检查文件是否存在，不存在则新建，存在则加载
            output_file = f"{output_dir}/ConvLSTMPINNUncertaintyWeighting_prediction_results.xlsx"
    ...
    def save_metrics_to_excel(metrics, output_path="ConvLSTMPINNmetrics_results.xlsx"):
    ...
    # 调用函数保存（替换为你的实际输出路径）
    save_metrics_to_excel(metrics, output_dir + "/ConvLSTMPINNmetrics_results_summary.xlsx")
    # 计算平均指标
    ...
    # 保存结果
    with open(f'{output_dir}/ConvLSTMPINNResults.txt', 'w') as f:
        for task in avg_metrics:
            f.write(f"\n{task.upper()}:\n")
            for metric, value in avg_metrics[task].items():
...
if __name__ == '__main__':
    ...
    file_path015 = '../dataset/015.xlsx'
    file_path02 = '../dataset/02.xlsx'
    ...
    StabilizationFactor = np.concatenate((data0100['StabilizationFactor'].array, data015['StabilizationFactor'].array,
                                          data02['StabilizationFactor'].array,
                                          data0025['StabilizationFactor'].array,
    ...
    # 测试数据路径
    # file_path005 = '../dataset/中雨005/0050.xlsx'
    # file_path0075 = '../dataset/中雨0075/0075.xlsx'
    # file_path0100 = '../dataset/大雨01/0100.xlsx'
    # file_path0125 = '../dataset/大雨0125/0125.xlsx'
    # file_path0150 = '../dataset/大雨0150/015.xlsx'
    # file_path0175 = '../dataset/暴雨0175/0175.xlsx'
    file_path0200 = '../dataset/暴雨0200/02.xlsx'
    # data = pd.read_excel(file_path005, sheet_name='Sheet2')
    # data = pd.read_excel(file_path0075, sheet_name='Sheet2')
    # data = pd.read_excel(file_path0100, sheet_name='Sheet2')
    # data = pd.read_excel(file_path0125, sheet_name='Sheet2')
    # data = pd.read_excel(file_path0150, sheet_name='Sheet2')
    # data = pd.read_excel(file_path0175, sheet_name='Sheet2')
    data = pd.read_excel(file_path0200, sheet_name='Sheet2')
    ...
    rtest_model(
        ...
        model_path='model_snapshot/ConvLSTMPINNMutilModelUnetStability_epoch_100.pth',
        output_dir='MutilControlConvLSTM结果图像0200'
    )