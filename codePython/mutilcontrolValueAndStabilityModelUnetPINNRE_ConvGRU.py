...
class ConvGRU(nn.Module):
    def __init__(self, input_channels, hidden_channels, kernel_size=3):
        ...
        # 重置门和更新门的卷积
        self.conv_xz = nn.Conv2d(input_channels, hidden_channels, kernel_size, padding=self.padding)
        self.conv_hz = nn.Conv2d(hidden_channels, hidden_channels, kernel_size, padding=self.padding, bias=False)
        self.conv_xr = nn.Conv2d(input_channels, hidden_channels, kernel_size, padding=self.padding)
        self.conv_hr = nn.Conv2d(hidden_channels, hidden_channels, kernel_size, padding=self.padding, bias=False)

        # 候选隐藏状态的卷积
        self.conv_xh = nn.Conv2d(input_channels, hidden_channels, kernel_size, padding=self.padding)
        self.conv_hh = nn.Conv2d(hidden_channels, hidden_channels, kernel_size, padding=self.padding, bias=False)
    ...
    def forward(self, x, hidden_state=None):
        ...
        # 计算候选隐藏状态
        h_tilde = torch.tanh(self.conv_xh(x) + self.conv_hh(r * h))
...
class SlopeNet(nn.Module):
    def __init__(self):
        ...
        # 替换为ConvGRU序列
        self.conv_gru_stack = nn.Sequential(
            ConvGRU(input_channels=16, hidden_channels=128, kernel_size=3),
            nn.MaxPool2d(2),  # 下采样
            ConvGRU(input_channels=128, hidden_channels=2, kernel_size=3),
            nn.MaxPool2d(2),  # 下采样
            ConvGRU(input_channels=2, hidden_channels=1, kernel_size=3)
        )
    ...
    def forward(self, x, featureinit, PorePressure, Stress):
        ...
        # 通过ConvGRU序列处理
        h, _ = self.conv_gru_stack[0](Stabilization)
        h = self.conv_gru_stack[1](h)
        h, _ = self.conv_gru_stack[2](h)
        h = self.conv_gru_stack[3](h)
        h, _ = self.conv_gru_stack[4](h)
    ...
    def get_parameter_groups(self):
        ...
        stabilization_params += list(self.conv_gru_stack.parameters())
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 100 == 0:
            ...
            save_mode_path = os.path.join(snapshot_path,
                                          'ConvGRUPINNMutilModelUnetStability_epoch_' +
                                          str(epoch + 1) + '.pth')
...
def rtest_model(datasetTest, InfiltrationStrength, model_path, output_dir='output', batch_size=1):
    ...
    with ((torch.no_grad())):
        for i, (x1, x2, x3, x4, y1, y2, y3, y5 ) in enumerate(dataloaderTest):
            ...
            output_img = outputs1[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/ConvGRUPINN饱和度StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            ...
            output_img = outputs2[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/ConvGRUPINN孔隙压力StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"prediction_{InfiltrationStrength[i]}_{(Time[i] + 1)}.png")
            plt.imsave(save_path, output_img)
            ...
            output_img = outputs3[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/ConvGRUPINN应力StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"prediction_{InfiltrationStrength[i]}_{(Time[i] + 1)}.png")
            plt.imsave(save_path, output_img)
            ...
            output_file = f"{output_dir}/ConvGRUPINNUncertaintyWeighting_prediction_results.xlsx"
    ...
    def save_metrics_to_excel(metrics, output_path="ConvGRUPINNmetrics_results.xlsx"):
    ...
    # 调用函数保存（替换为你的实际输出路径）
    save_metrics_to_excel(metrics, output_dir + "/ConvGRUPINNmetrics_results_summary.xlsx")
    # 计算平均指标
    ...
    # 保存结果
    with open(f'{output_dir}/ConvGRUPINNResults.txt', 'w') as f:
        for task in avg_metrics:
            f.write(f"\n{task.upper()}:\n")
            for metric, value in avg_metrics[task].items():
...
if __name__ == '__main__':
    ...
    rtest_model(
        ...
        model_path='model_snapshot/ConvGRUPINNMutilModelUnetStability_epoch_100.pth',
        output_dir='MutilControlConvGRU结果图像0200'
    )