...
class SlopeNet(nn.Module):
    def __init__(self):
        ...
        self.log_var3 = nn.Parameter(torch.zeros(1))  # 任务2的噪声参数
    ...
    def forward(self, x, featureinit, PorePressure, Stress):
        ...
        # 融合特征
        x = x + self.log_phvar1 * feature
        PorePressureControl = PorePressureControl + self.log_phvar2 * feature
        StressControl = StressControl + self.log_phvar3 * feature
        ...
        # 稳定性预测部分支路连接点
        Stabilization = StressControl + PorePressureControl
...
def modelLoss(net, XTrain1, XTrain2, XTrain3, XTrain4, Y1, Y2, Y3, Y5, epoch, i):
    ...
    # 1. 有效应力原理约束
    stress_loss = criterion(torch.from_numpy(P_outputs3[..., :200, :200] +
                                                   P_outputs1[..., :200, :200] * P_outputs2[..., :200,
                                                                                 :200]).float().to(device),
                                  torch.from_numpy(P_Y3[..., :200, :200] +
                                                   P_Y1[..., :200, :200] * P_Y2[..., :200, :200]).float().to(
                                      device)) / (400000 * 20000)
    ...
    # 动态调整损失权重
    loss = (torch.exp(-net.log_var1) * (loss1 + loss2 + loss3) +
            torch.exp(-net.log_var3) * (stress_loss + darcy_loss + continuity_loss) +
            torch.exp(-net.log_var2) * loss5)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 100 == 0:
            ...
            save_mode_path = os.path.join(snapshot_path,
                                          'NonInPhysicBaselineMutilModelUnetStability_epoch_' +
                                          str(epoch + 1) + '.pth')
...
def rtest_model(datasetTest, InfiltrationStrength, model_path, output_dir='output', batch_size=1):
    ...
    with ((torch.no_grad())):
        for i, (x1, x2, x3, x4, y1, y2, y3, y5 ) in enumerate(dataloaderTest):
            ...
            output_img = outputs1[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/NonInPhysicBaseline饱和度StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            ...
            output_img = outputs2[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/NonInPhysicBaseline孔隙压力StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"prediction_{InfiltrationStrength[i]}_{(Time[i] + 1)}.png")
            plt.imsave(save_path, output_img)
            ...
            output_img = outputs3[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/NonInPhysicBaseline应力StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"prediction_{InfiltrationStrength[i]}_{(Time[i] + 1)}.png")
            plt.imsave(save_path, output_img)
            ...
            output_file = f"{output_dir}/NonInPhysicBaselineUncertaintyWeighting_prediction_results.xlsx"
    ...
    # 调用函数保存（替换为你的实际输出路径）
    save_metrics_to_excel(metrics, output_dir + "/NonInPhysicBaselinemetrics_results_summary.xlsx")
    ...
    # 保存结果
    with open(f'{output_dir}/NonInPhysicBaselineResults.txt', 'w') as f:
        for task in avg_metrics:
            f.write(f"\n{task.upper()}:\n")
            for metric, value in avg_metrics[task].items():
...
if __name__ == '__main__':
    ...
    rtest_model(
        ...
        model_path='model_snapshot/NonInPhysicBaselineMutilModelUnetStability_epoch_100.pth',
