    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 100 == 0:
            ...
            save_mode_path = os.path.join(snapshot_path,
                                          'BaselineMutilModelUnetStability_epoch_' +
                                          str(epoch + 1) + '.pth')
...
def rtest_model(datasetTest, InfiltrationStrength, model_path, output_dir='output', batch_size=1):
    ...
    with ((torch.no_grad())):
        for i, (x1, x2, x3, x4, y1, y2, y3, y5 ) in enumerate(dataloaderTest):
            ...
            output_img = outputs1[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/Baseline饱和度StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            ...
            output_img = outputs2[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/Baseline孔隙压力StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"prediction_{InfiltrationStrength[i]}_{(Time[i] + 1)}.png")
            plt.imsave(save_path, output_img)
            ...
            output_img = outputs3[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/Baseline应力StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"prediction_{InfiltrationStrength[i]}_{(Time[i] + 1)}.png")
            plt.imsave(save_path, output_img)
            ...
            output_file = f"{output_dir}/BaselineUncertaintyWeighting_prediction_results.xlsx"
    ...
    # 调用函数保存（替换为你的实际输出路径）
    save_metrics_to_excel(metrics, output_dir + "/Baselinemetrics_results_summary.xlsx")
    ...
    # 保存结果
    with open(f'{output_dir}/BaselineResults.txt', 'w') as f:
        for task in avg_metrics:
            f.write(f"\n{task.upper()}:\n")
            for metric, value in avg_metrics[task].items():
...
if __name__ == '__main__':
    ...
    rtest_model(
        ...
        model_path='model_snapshot/BaselineMutilModelUnetStability_epoch_100.pth',
        output_dir='MutilControl结果图像0075'
    )
