...
class SlopeNet(nn.Module):
    def __init__(self):
        ...
        self.log_var3 = nn.Parameter(torch.zeros(1))  # 任务3的噪声参数
    ...
    def forward(self, x, featureinit, PorePressure, Stress):
        ...
        # print(f"feature shape after FeatureModule: {feature.shape}")
        ...
        temp = x * feature
        x = x + temp
        PorePressureControl = (PorePressureControl + PorePressureControl * feature + self.compute_Saturability_PorePressure(x)) / 2
        StressControl = StressControl + (StressControl * feature + x * PorePressureControl) / 2
        # print(PorePressureControl) + self.compute_Saturability_PorePressure(temp)
    ...
    # 新增方法：将参数分为共享组和 Stabilization 组
    def get_parameter_groups(self):
        ...
        stabilization_params += list(self.fc1.parameters())
        stabilization_params += list(self.fc2.parameters())
...
def modelLoss(net, XTrain1, XTrain2, XTrain3, XTrain4, Y1, Y2, Y3, Y5, epoch, i):
    ...
    # 只对可训练参数计算梯度
    if trainable_params:
        gradients = torch.autograd.grad(loss, trainable_params, retain_graph=True, create_graph=True, allow_unused=True)
    else:
        gradients = []
    # ==================== 修改结束 ====================
...
def train_model(datasetTrain, model_save_dir='models', batch_size=4, epochs=500, lr=0.01,
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0 & epoch < 100:
            # 模型训练好之后的保存路径
            snapshot_path = model_save_dir
            os.makedirs(snapshot_path, exist_ok=True)
            save_mode_path = os.path.join(snapshot_path,
                                          'PINNMutilModelUnetStability_FineTuning_epoch_' +
                                          str(epoch + 1) + '.pth')
...
def rtest_model(datasetTest, InfiltrationStrength, model_path, output_dir='output', batch_size=1):
    ...
    with ((torch.no_grad())):
        for i, (x1, x2, x3, x4, y1, y2, y3, y5 ) in enumerate(dataloaderTest):
            ...
            output_img = outputs1[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/PINN饱和度StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            ...
            output_img = outputs2[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/PINN孔隙压力StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"prediction_{InfiltrationStrength[i]}_{(Time[i] + 1)}.png")
            plt.imsave(save_path, output_img)
            ...
            output_img = outputs3[0].detach().cpu().numpy().transpose(1, 2, 0)
            output_img = (output_img * 255).astype(np.uint8)
            save_dir = f"{output_dir}/PINN应力StabilityUncertaintyWeighting"
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"prediction_{InfiltrationStrength[i]}_{(Time[i] + 1)}.png")
            plt.imsave(save_path, output_img)
            ...
            output_file = f"{output_dir}/PINNUncertaintyWeighting_prediction_results.xlsx"
    ...
    def save_metrics_to_excel(metrics, output_path="PINNmetrics_results.xlsx"):
    ...
    # 调用函数保存（替换为你的实际输出路径）
    save_metrics_to_excel(metrics, output_dir + "/PINNmetrics_results_summary.xlsx")
    # 计算平均指标
    ...
    # 保存结果
    with open(f'{output_dir}/PINNResults.txt', 'w') as f:
        for task in avg_metrics:
            f.write(f"\n{task.upper()}:\n")
            for metric, value in avg_metrics[task].items():
...
if __name__ == '__main__':
    ...
    # 监测训练数据路径
    file_path0100 = '../dataset_monitor/0100.xlsx'
    file_path015 = '../dataset_monitor/0150.xlsx'
    file_path02 = '../dataset_monitor/0200.xlsx'
    data0100 = pd.read_excel(file_path0100, sheet_name='Sheet4')
    data0150 = pd.read_excel(file_path015, sheet_name='Sheet4')
    data0200 = pd.read_excel(file_path02, sheet_name='Sheet4')
    Time = np.concatenate((data0100['TIME0'].array / 60, data0150['TIME0'].array / 60, data0200['TIME0'].array / 60,))
    InfiltrationStrength = np.concatenate((data0100['CLASS'].array, data0150['CLASS'].array, data0200['CLASS'].array,
                                           ))
    StabilizationFactor = np.concatenate((data0100['StabilizationFactor'].array, data0150['StabilizationFactor'].array,
                                          data0200['StabilizationFactor'].array,))
    # 数据集路径
    x1_path = "../dataset_monitor/SaturabilityTraincopy"
    x21_values = InfiltrationStrength  # 降雨等级
    x22_values = Time  # 入渗时间或降雨时长
    x2_values = np.column_stack((x21_values, x22_values))
    x3_path = "../dataset_monitor/PorePressureControlTraincopy"
    x4_path = "../dataset_monitor/StressControlTraincopy"

    y1_path = "../dataset_monitor/Saturabilitycopy"
    y2_path = "../dataset_monitor/PorePressureControlcopy"
    y3_path = "../dataset_monitor/StressControlcopy"
    ...
    # 训练模型 - 现在使用预训练权重并冻结编码器
    # train_history = train_model(
    #     datasetTrain=datasetTrain,
    #     model_save_dir="model_snapshot_finetuned_MH",
    #     batch_size=4,
    #     epochs=1000,
    #     lr=0.001,
    #     pretrained_path="model_snapshot/PINNMutilModelUnetStability_epoch_100.pth",  # 预训练模型路径
    #     freeze_encoder=True  # 冻结编码器参数
    # )

    # 测试数据路径
    data = pd.read_excel('../dataset_monitor/大雨0100/0100.xlsx', sheet_name='Sheet4')
    # data = pd.read_excel('../dataset_monitor/大雨0125/0125.xlsx', sheet_name='Sheet4')
    # data = pd.read_excel('../dataset_monitor/大雨0150/0150.xlsx', sheet_name='Sheet4')
    # data = pd.read_excel('../dataset_monitor/暴雨0175/0175.xlsx', sheet_name='Sheet4')
    # data = pd.read_excel('../dataset_monitor/暴雨0200/0200.xlsx', sheet_name='Sheet4')
    Time = data['TIME0'].array / 60
    InfiltrationStrength = data['CLASS'].array
    StabilizationFactor = data['StabilizationFactor'].array
    # 测试数据集路径
    name = '大雨0100'
    x1_path = f"../dataset_monitor/{name}/SaturabilityTraincopy"
    x21_values = InfiltrationStrength  # 降雨等级
    x22_values = Time  # 入渗时间或降雨时长
    x2_values = np.column_stack((x21_values, x22_values))
    x3_path = f"../dataset_monitor/{name}/PorePressureControlTraincopy"
    x4_path = f"../dataset_monitor/{name}/StressControlTraincopy"

    y1_path = f"../dataset_monitor/{name}/Saturabilitycopy"
    y2_path = f"../dataset_monitor/{name}/PorePressureControlcopy"
    y3_path = f"../dataset_monitor/{name}/StressControlcopy"
    y5_values = StabilizationFactor  # 稳定性
    # 加载测试数据以及批次设置
    ...
    rtest_model(
        ...
        model_path='model_snapshot_finetuned_MH/PINNMutilModelUnetStability_FineTuning_epoch_1000.pth',
        output_dir='MutilControl结果图像0100_FineTuning'
    )